 ...
 */
 ...
 function moveDiv(TooltipDivID)
  {
   ...
   // offset down and to the right of mouse cursor
   eX = cX+10;
   eY = cY+15;

   var minMargin = 10;
   var tt = document.getElementById(currentTooltipDivID);
   var ttw = 0;
   var tth = 0;
   if(tt != null)
    { ttw = tt.clientWidth; tth = tt.clientHeight; }

   // how wide/tall is the view port, minus the width/height of the tool tip and margin
   var xtreme = (window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth) - ttw - minMargin;
   var ytreme = (window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight) - tth - minMargin;

   // When the mouse is too far to the right or down, move
   // the tool tip to the other side of the mouse
   if(eX > xtreme)
    eX -= ttw + 1.5 * minMargin;
   if(eY > ytreme)
    eY -= tth + 1.5 * minMargin;
 ...
 function addArea(imageMapID,shapeType,coordsList,actionOver,actionOut,hrefOrOnClick)
  {
   ...
   if(hrefOrOnClick != '')
   {
     var opt = hrefOrOnClick.split("=");
     var fn = opt[0];
     opt.splice(0,1);
     var y = opt.join("=");
     if(fn == "onclick")
     {
       element.onclick = function() { eval(y); };
       element.href = '#'
     }
     else if(fn == "href")
       element.href = y;
   ...
   for(i=0;i<=Zones.length-2;i++)
    {
     ...
     if(!Options[5])
     	Options[5] = '';
     addArea(ImageMapID,Options[0],Options[1],'showDiv("toolTipDiv","'+Options[2]+'","'+Options[3]+'","'+Options[4].replace('"','')+'");','hideDiv("toolTipDiv");',Options[5]);
    }
  }
